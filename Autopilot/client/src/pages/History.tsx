...
//                     <Button className="w-40 justify-start text-left font-normal bg-white border border-gray-300 hover:bg-gray-50 text-gray-700">
...
//               <Button className="bg-[#1a785f] hover:bg-[#1e896d] font-semibold" onClick={clearFilters}>
//                 Clear Filters
//               </Button>
//             </div>

//             {/* Table */}
//             <div className="overflow-x-auto rounded-2xl border border-[#059472]">
//               <table className="w-full text-md text-left">
//                 <thead className="bg-[#06a57f] text-white font-semibold">
//                   <tr>
...
//                           <span className="absolute left-7 top-1/2 -translate-y-1/2 z-20 hidden group-hover:flex flex-col min-w-[180px] bg-white border border-[#C4D3FF] shadow-xl p-2 text-gray-900 text-xs mt-2">
...
export default function History() {
  ...
  // Mock data for testing
  const mockTrades: TradeData[] = [
    { CreatedAt: "2025-08-15T10:30:00Z", Side: "buy", Size: 2, State: "filled", AverageFillPrice: "2800", PaidCommission: "5", Symbol: "ETHUSD" },
    { CreatedAt: "2025-08-16T14:45:00Z", Side: "sell", Size: 1, State: "filled", AverageFillPrice: "42000", PaidCommission: "10", Symbol: "BTCUSD" },
    { CreatedAt: "2025-08-17T09:10:00Z", Side: "buy", Size: 5, State: "cancelled", AverageFillPrice: "41000", PaidCommission: "0", Symbol: "BTCUSD" },
    { CreatedAt: "2025-08-15T10:30:00Z", Side: "buy", Size: 2, State: "filled", AverageFillPrice: "2800", PaidCommission: "5", Symbol: "ETHUSD" },
    { CreatedAt: "2025-08-16T14:45:00Z", Side: "sell", Size: 1, State: "filled", AverageFillPrice: "42000", PaidCommission: "10", Symbol: "BTCUSD" },
  ];
      ...
      <div className="flex-1 md:ml-[14rem]">
        ...
        <div className="min-h-screen bg-gray-50 dark:bg-[#2d3139] p-6">
          <div className="max-w-8xl mx-auto">
            ...
            <h1 className="text-2xl font-semibold text-gray-900 dark:text-foreground mb-6">History</h1>
            ...
            <div className="flex flex-wrap justify-between rounded-lg gap-4 items-center mb-6">
              <div className="flex items-center gap-2">
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button variant="outline" className="w-60 justify-center text-center font-normal bg-card text-foreground hover:bg-muted">
                      <CalendarIcon className="mr-2 h-4 w-4" />
                      <span className="truncate">
                        {startDate ? format(startDate, "MMM d, yyyy") : "Pick a date"}
                      </span>
            ...
            <div className="overflow-x-auto rounded-2xl border border-[#06a57f]">
              <table className="w-full text-md text-left">
                  ...
                  <tr>
                    ...
                    <th className="py-3 px-4">
                      <div className="flex items-center">
                        ...
                        <span className="relative group ml-[16px] mb-1 inline-block align-middle">
                          <Info className="w-4 h-4 mt-1 cursor-pointer" />
                          <span className="absolute left-7 top-1/2 -translate-y-1/2 z-20 hidden group-hover:flex flex-col min-w-[180px] bg-popover border border-border shadow-xl p-2 text-popover-foreground text-xs mt-2">
                            <span className="text-muted-foreground py-2">Lot Size Info</span>
                            <span className="font-semibold mb-1">1 lot = 0.01 ETH or 0.001 BTC.</span>
                            <span className="absolute -left-2 top-1/2 -translate-y-1/2 w-0 h-0 border-t-8 border-b-8 border-r-8 border-transparent border-r-border"></span>
                          </span>
                        </span>
                      </div>
                    </th>
                ...
                <tbody>
                  {filteredTrades.length > 0 ? (
                    filteredTrades.map((trade, index) => (
                      <tr key={index} className="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-muted">
                        <td className="py-3 px-4 text-gray-700 dark:text-foreground font-medium">
                          {format(new Date(trade.CreatedAt), "yyyy-MM-dd")}
                          <br />
                          <span className="text-gray-500 dark:text-muted-foreground font-medium text-[14px]">{format(new Date(trade.CreatedAt), "HH:mm:ss")}</span>
                        </td>
                        <td className="py-3 px-4">
                          <div className="flex items-center">
                            <div className={`w-1 h-8 rounded-full mr-3 ${trade.Side === "buy" ? "bg-green-500" : "bg-red-500"}`}></div>
                            <div>
                              <div className="font-medium text-gray-800 dark:text-foreground">{trade.Symbol}</div>
                              <div className={`text-[14px] ${trade.Side === "buy" ? "text-green-600" : "text-red-600"}`}>
                                {trade.Side.charAt(0).toUpperCase() + trade.Side.slice(1)}
                              </div>
                            </div>
                        ...
                        <td className="py-3 px-4 font-medium text-gray-800 dark:text-foreground">{trade.Size.toFixed(2)} <span className="text-gray-500 dark:text-muted-foreground">{trade.Symbol.replace('USD', '')}</span></td>
                        <td className="py-3 px-4 font-medium text-gray-800 dark:text-foreground">{parseFloat(trade.AverageFillPrice).toLocaleString()}</td>
                        <td className="py-3 px-4">
                          <span className={`${trade.State === "filled"
                        ...
                        <td className="py-3 px-4 font-medium text-gray-800 dark:text-foreground text-right">{parseFloat(trade.PaidCommission).toFixed(2)} <span className="text-gray-500 dark:text-muted-foreground">USDT</span></td>
                      </tr>
                    ))
                  ) : (
                    <tr>

                      <td colSpan={6} className="p-4 text-center text-gray-500 dark:text-muted-foreground">No trades found</td>
                    </tr>
                  )}
                </tbody>
              </table>
            ...
            <div className="flex items-center justify-end pt-4">
              ...
              <span className="mx-4 text-sm font-medium text-gray-700 dark:text-muted-foreground">Page {page}</span>
              <Button
                size="sm"
                disabled={!nextPage}
                onClick={() => nextPage && setPage(nextPage)}
