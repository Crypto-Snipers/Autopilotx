...
# SMA_DISTANCE_PERCENTAGE = 1  # Minimum distance from SMA200 as percentage
...
#         df = calculate_supertrend(df)  # Add SuperTrend calculation
...
#         # Get all trades for today that resulted in a loss
#         today_losing_trades = trade_collection.find(
#             {
#                 "Strategy": STRATEGY,
#                 "Type": "Exit",
#                 "OrderTime": {"$gte": today_start},
#                 "PnL": {"$lt": 0},
#             }
#         )

#         daily_loss = sum(abs(trade.get("PnL", 0)) for trade in today_losing_trades)

#         if daily_loss >= MAX_DAILY_LOSS:
#             logger.warning(
#                 f"Max daily loss of ${MAX_DAILY_LOSS} exceeded. Current loss: ${daily_loss:.2f}. No new trades will be taken today."
#             )
#             return
...
#             elif current_time.minute % TF == (TF - 1) and current_time.second >= 58:
...
trade_config = trade_config.find_one({"SYMBOL": "BTC"})
...
trade_collection = db["trades"]
position_collection = db["position"]
TF = trade_config.get("tf", 5)  # 5-minute timeframe
TF_MINUTES = TF - 1
TF_SECONDS = 59
TIMEFRAME = f"{TF}min"

# Strategy Parameters - Configurable
PATTERN_PERCENTAGE = trade_config.get("PATTERN_PERCENTAGE", 0.3)  # Minimum percentage change for consecutive candle pattern
SMA_DISTANCE_PERCENTAGE = trade_config.get("SMA_DISTANCE_PERCENTAGE", 0.5)  # Minimum distance from SMA200 as percentage
STOPLOSS_BUFFER_PERCENTAGE = trade_config.get("STOPLOSS_BUFFER_PERCENTAGE", 0.1)  # Buffer percentage for stoploss
RISK_REWARD_RATIO = trade_config.get("RISK_REWARD_RATIO", 3.0)  # Target is 3x the stoploss distance
TRAIL_RATIO = trade_config.get("TRAIL_RATIO", 2.0)  # Trail point is 2x the stoploss distance
TARGET_RATIO_FINAL = trade_config.get("TARGET_RATIO_FINAL", 5.0)  # Final target is 5x the stoploss distance
EXIT_1_PERCENTAGE = trade_config.get("EXIT_1_PERCENTAGE", 50)  # Percentage of position to exit at first target (50% default)

# Indicator parameters
SMA_PERIOD = trade_config.get("SMA_PERIOD", 200)  # SMA period
