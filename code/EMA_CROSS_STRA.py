...
from code.coindcx_order import MONGO_DB_NAME
...
def fetch_historical_data(timeframe):
    try:
        ...
        candleData = list(candles.find({"symbol": candleSymbol}, {"_id": 0}).sort("timestamp", pymongo.ASCENDING).limit(40000))
        ...
        missing_columns = [col for col in required_columns if col not in candleDf.columns]
...
def check_buy_signal(df):
    ...
    return (
        prev_row['P_EMA_20'] < prev_row['P_EMA_50'] and  # # buy
        last_row['EMA_20'] > last_row['EMA_50'] and
        last_row['close'] > last_row['EMA_20'] and
        last_row['close'] > last_row['EMA_50']
    )
...
def live_trading_loop():
    ...
    while True:
        try:
            ct = datetime.now().replace(second=0, microsecond=0, minute=(datetime.now().minute // TF) * TF)
            next_candle_time = ct + timedelta(minutes=TF)

            open_positions = list(PositionCollection.find({"Strategy":STRATEGY, "Status":"Open"}))
            ...
            if len(open_positions) == 0:
                ...
                if check_buy_signal(df) and len(open_positions) == 0:
                    ...
                    LiveCollection.update_one({"Strategy":STRATEGY, "ID":POSITION_ID}, {"$set": live_status_update_data}, upsert=True)
                ...
                for position in open_positions:
                    ...
                    if side == "BUY":
                        if current_price <= position['StopLoss']:
                            ...
                            logger.info(f"Position {position_id}: candle close ({df['close'].iloc[-2]:.2f}) < EMA20 ({df['EMA_20'].iloc[-2]:.2f}). Exiting at {exit_price:.2f}")
                        ...
                        {"Strategy":STRATEGY, "ID":position_id},
                        {"$set":{
                            "ExitTime": datetime.utcnow(),
                            "ExitPrice": exit_price,
                            "ExitType": exit_type,
...
if __name__ == "__main__":
    ...
    logger.info(dt, next_candle_time)
