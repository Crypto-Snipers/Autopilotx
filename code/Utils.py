...
from os import sep, path, pardir
import os
import logging
...
def setup_logger(
    ...
    if log_file is None:
        log_file = os.path.join(f"/home/ubuntu/cryptocode-{ENV}/logs", f"{name}.log")
    # Create logger
    ...
    if log_to_file:
        ...
        if log_dir and not os.path.exists(log_dir):
            try:
                ...
                logger.error(
                    f"Permission denied when creating log directory: {log_dir}"
                )
                return logger
...
def kill_me(screen_name):
    ...
    screen = Run(["screen", "-ls"], stdout=Pipe, text=True).stdout
    # Match any length PID for consistency with RunScreens
    pids = FindAll(rf"([0-9]+)\.{screen_name}\s", screen)
...
#     Command = f"screen -dmSL {ID} -Logfile {LogPath} /home/ubuntu/cryptocode/venv/bin/python3 {CodePath}"
#     print(Command)
...
def code_runner(ID, CodePath, LogPath, Params, logger) -> None:
    """Runs the Code With the Given Parameters"""
    ENV = os.getenv("ENV", "dev")
    if Params and Params[0].endswith("uvicorn"):
        # Full custom command mode (ignore CodePath)
        Command = f"screen -dmSL {ID} -Logfile {LogPath} " + " ".join(Params)
    else:
        # Default Python file execution
        Command = f"screen -dmSL {ID} -Logfile {LogPath} /home/ubuntu/cryptocode-{ENV}/venv/bin/python3 {CodePath}"
        for Param in Params:
            Command += f" {Param}"
...
def file_run(filename, logger, logname=None, params=[]):
    # Generate a random 5-digit number for the screen/session name
    random_id = str(random.randint(1000, 9999))
    logname = logname if logname else f"{filename}_{random_id}"
    ...
    code_runner(
        ID=logname, CodePath=codepath, LogPath=logpath, Params=params, logger=logger
    )
