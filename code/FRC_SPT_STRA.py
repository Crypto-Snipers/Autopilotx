...
def detect_fractals(df: pd.DataFrame, lookback_periods: int = 2) -> pd.DataFrame:
    ...
    df_result["fractal_time_top"] = pd.to_datetime(df_result["fractal_time_top"], utc=True)
    df_result["fractal_time_bottom"] = pd.to_datetime(df_result["fractal_time_bottom"], utc=True)
...
def analyze_market_data(df, ATR_PERIOD, FRACTAL_PERIOD, ST_FACTOR):
    df = df.copy()
...
def check_for_entry_signals(df, trade_dict):
    ...
    if (
        float(ticker["high"]) > float(latest["fractal_top"])
        and latest_time >= latest["fractal_time_top"]
        and float(prev_frac) != float(latest["fractal_top"])
        and float(latest["close"]) > float(latest["Supertrend"])
    ):
        ...
        entry_price = float(ticker["close"])
        ...
        return {
            ...
            "FractalPrice": float(latest["fractal_top"]),
            "FractalTime": latest["fractal_time_top"],
    ...
    if (
        float(ticker["low"]) < float(latest["fractal_bottom"])
        and latest_time >= latest["fractal_time_bottom"]
        and float(prev_frac) != float(latest["fractal_bottom"])
        and float(latest["close"]) < float(latest["Supertrend"])
    ):
        ...
        return {
            ...
            "Target": float(take_profit),
            "Atr": float(latest["atr"]),
            "FractalPrice": float(latest["fractal_bottom"]),
            "FractalTime": latest["fractal_time_bottom"],  # #FratalTime
            "EntryTime": latest_time,
            "Direction": int(direction),
            "SuperTrend": int(latest["Supertrend"]),
        }
...
def main():
    ...
    while True:
        try:
            ...
            df = analyze_market_data(df, ATR_PERIOD=ATR_PERIOD, FRACTAL_PERIOD=FRACTAL_PERIOD, ST_FACTOR=ST_FACTOR)
            # logger.info(df.loc[df.index[-3:], ['date','fractal_top', 'fractal_bottom', 'fractal_time_top', 'fractal_time_bottom']].to_dict())
            # sys.exit(1)
