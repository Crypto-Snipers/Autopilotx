...
class CoinDcxClient:
    ...
    def _send_request(self, method, path, body=None):
        ...
        try:
            response = requests.request(
                method, url, data=json_body, headers=headers, timeout=30
            )
    ...
    def get_positions(
        ...
        body = {"page": str(page), "size": str(size)}
        ...
        if not any([pairs, position_ids, margin_currency_short_name is not None]):
            raise ValueError(
                "Either 'pairs', 'position_ids', or 'margin_currency_short_name' must be provided"
            )

        return self._send_request(
            "POST", "/exchange/v1/derivatives/futures/positions", body
        )
    ...
    def get_futures_instrument(self, pair, margin_currency_short_name="USDT"):
        ...
        return self._send_request(
            "GET", "/exchange/v1/derivatives/futures/data/instrument", params
        )
    ...
    def get_futures_trades(self, pair):
        ...
        params = {"pair": pair}
        return self._send_request(
            "GET", "/exchange/v1/derivatives/futures/data/trades", params
        )
    ...
    def exit_position(self, position_id: str) -> dict:
        ...
        Example:
            ...
            >>> print(result)
        """
        body = {"timestamp": int(time.time() * 1000), "id": position_id}

        return self._send_request(
            "POST", "/exchange/v1/derivatives/futures/positions/exit", body
        )
    ...
    def get_futures_orderbook(self, instrument, depth=50):
        ...
        """
        return self._send_request(
            "GET", f"/market_data/v3/orderbook/{instrument}/{depth}", None
        )
    ...
    def create_futures_order(
        ...
        margin_currency_short_name = (
            margin_currency_short_name.upper() if margin_currency_short_name else "USDT"
        )
        timestamp = int(time.time() * 1000)
        ...
        if (
            order_type
            in ["stop_limit", "stop_market", "take_profit_limit", "take_profit_market"]
        ...
        body = {"timestamp": timestamp, "order": order}

        print(f"Creating order with params: {body}")
        return self._send_request(
            "POST", "/exchange/v1/derivatives/futures/orders/create", body
        )
    ...
    def cancel_futures_order(self, order_id=None, client_order_id=None):
        ...
        return self._send_request(
            "POST", "/exchange/v1/derivatives/futures/orders/cancel", body
        )

    def get_futures_orders(
        self, status="open", side=None, page=1, size=10, margin_currency_short_name=None
    ):
        """
        ...
        Returns:
            list: List of orders
        """
        body = {"status": status, "page": str(page), "size": str(size)}
        ...
        return self._send_request(
            "POST", "/exchange/v1/derivatives/futures/orders", body
        )
    ...
    def get_futures_order_status(self, order_id=None, client_order_id=None):
        ...
        return self._send_request(
            "POST", "/exchange/v1/derivatives/futures/orders/status", body
        )
    ...
    def cancel_all_futures_orders(self, pair=None, side=None):
        ...
        return self._send_request(
            "POST", "/exchange/v1/derivatives/futures/orders/cancel_all", body
        )

    def get_futures_trade_history(
        self, pair=None, from_id=None, limit=50, from_timestamp=None, to_timestamp=None
    ):
        """
        ...
        return self._send_request(
            "POST", "/exchange/v1/derivatives/futures/trades", body
        )

    def get_futures_balance(self, margin_currency_short_name=None):
        """
        ...
        signature = hmac.new(
            (
                self.secret_key
                if isinstance(self.secret_key, bytes)
                else bytes(self.secret_key, encoding="utf-8")
            ),
            json_body.encode(),
            hashlib.sha256,
        ).hexdigest()
    ...
    def transfer_funds(self, amount, currency, from_account, to_account):
        ...
        return self._send_request(
            "POST", "/exchange/v1/derivatives/futures/transfer", body
        )
    ...
    def get_balances(self):
        ...
        try:
            ...
            signature = hmac.new(
                self.secret_key, json_body.encode("utf-8"), hashlib.sha256
            ).hexdigest()
            ...
            response = requests.post(
                f"{self.base_url}{path}", data=json_body, headers=headers
            )
            ...
            raise CoinDcxRequestError(
                f"Network error while fetching balances: {str(e)}"
            ) from e
    ...
    def create_order(
        ...
        if order_type not in ["limit_order", "market_order"]:
            raise CoinDcxError(
                "order_type must be either 'limit_order' or 'market_order'"
            )
        ...
        body = {
            ...
            "total_quantity": str(
                total_quantity
            ),  # Convert to string as some APIs expect string values
        }
    ...
    def get_trade_history(
        ...
        body = {"symbol": symbol, "limit": limit, "sort": sort}
        if from_id is not None:
            body["from_id"] = from_id
        if from_timestamp is not None:
            body["from_timestamp"] = from_timestamp
...
class CoinDcxWebSocketClient:
    ...
    def __init__(
        ...
        )  # Enable socket.io logs for debugging
    ...
    def _generate_auth_signature(self, channel_name="coindcx"):
        if not self.secret_key:
            self.logger.warning(
                "Secret key not provided. Cannot generate auth signature."
            )
            return None
        ...
        def connect():
            ...
            for sub_type, channel_name, callback in self.subscriptions:
                self._subscribe_to_channel(
                    sub_type, channel_name, callback, is_reconnect=True
                )
    ...
    def _subscribe_to_channel(
        self, subscription_type, channel_name, user_callback, is_reconnect=False
    ):
        payload = {"channelName": channel_name}
        if subscription_type == "private":  # e.g., 'coindcx' for account updates
            if not self.api_key or not self.secret_key:
                self.logger.error(
                    f"API key and secret required for private channel {channel_name}"
                )
                return
            signature = self._generate_auth_signature(
                channel_name
            )  # or a generic one if needed
            if not signature:
                return
        ...
        if self.sio.connected:
            ...
            self.logger.warning(
                f"WebSocket not connected. Channel {channel_name} will be joined after connection."
            )
    ...
    def subscribe_to_ltp(self, instrument_id, callback):
        ...
        def _on_price_change(data):
            ...
            if data.get("channel", "").startswith(instrument_id):  # Basic check
                ...
                "s" in data and data["s"] == instrument_id
            ):  # For 'new-trade' like structure
                callback(data)
            elif (
                "data" in data
                and isinstance(data["data"], dict)
                and data["data"].get("s") == instrument_id
            ):
                callback(data["data"])
            ...
            # This part might need refinement based on actual message structure for price-change
                ...
                self.logger.debug(
                    f"Received {event_name} for {instrument_id} (no explicit symbol in root): {data}"
                )
                callback(data)  # Pass full data if symbol not directly in root

        self._subscribe_to_channel("public", channel_name, callback)
        self.logger.info(
            f"Subscribed to LTP for {instrument_id} on channel {channel_name}, event {event_name}"
        )
    ...
    def subscribe_to_candles(self, instrument_id, interval, callback):
        ...
        def _on_candlestick(data):
            ...
            if data.get("channel") == channel_name:
                callback(
                    data.get("data")
                )  # The actual candle data is in 'data' field of the response

        self._subscribe_to_channel("public", channel_name, callback)
        self.logger.info(
            f"Subscribed to {interval} candles for {instrument_id} on channel {channel_name}, event {event_name}"
        )
